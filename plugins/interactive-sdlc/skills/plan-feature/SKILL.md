...
Plan a feature with comprehensive codebase exploration, architecture design, milestone breakdown, and task decomposition that enables incremental delivery. This skill explores the codebase to understand existing patterns, gathers requirements, and creates a structured plan with logical milestones.

## Arguments

### Definitions

- **`[--explore N]`** (optional): Override default explore agent count. Defaults to configuration value (`defaultExploreAgents.feature`, default: 3).
- **`[--git]`** (optional): Commit plan file after creation.
- **`[context]`** (optional): Freeform context for argument inference (e.g., feature description, requirements).
...
- `defaultExploreAgents.feature` (default: `3`): Number of explore agents to launch
...
{
  "success": true,
  "planPath": "specs/feature-example-title.md",
  "feature": {
    "title": "Feature title",
    "milestones": 3,
    "totalTasks": 12,
