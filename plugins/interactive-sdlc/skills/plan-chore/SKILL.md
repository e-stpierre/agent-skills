...
Plan a maintenance task with comprehensive codebase exploration, gathering user requirements, and generating a structured plan document that guides implementation. This skill explores the codebase to understand existing patterns and conventions before creating a detailed plan.

## Arguments

### Definitions

- **`[--explore N]`** (optional): Override default explore agent count. Defaults to configuration value (`defaultExploreAgents.chore`, default: 2).
- **`[--git]`** (optional): Commit plan file after creation.
- **`[context]`** (optional): Freeform context for argument inference (e.g., chore title, requirements).
...
- `defaultExploreAgents.chore` (default: `2`): Number of explore agents to launch
...
{
  "success": true,
  "planPath": "specs/chore-example-title.md",
  "chore": {
    "title": "Chore title",
    "tasksIdentified": 5,
