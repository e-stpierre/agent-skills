...
Plan a bug fix with thorough root cause analysis, reproduction steps, and a structured fix strategy that prevents regression. This skill explores the codebase to understand the bug context, identifies the root cause, and generates a comprehensive plan document.

## Arguments

### Definitions

- **`[--explore N]`** (optional): Override default explore agent count. Defaults to configuration value (`defaultExploreAgents.bug`, default: 2).
- **`[--git]`** (optional): Commit plan file after creation.
- **`[context]`** (optional): Freeform context for argument inference (e.g., bug description, reproduction steps).
...
- `defaultExploreAgents.bug` (default: `2`): Number of explore agents to launch
...
{
  "success": true,
  "planPath": "specs/bug-example-title.md",
  "bug": {
    "title": "Bug title",
    "rootCause": "Brief technical explanation",
    "fixStrategy": "High-level approach",
    "testCases": 3
  }
}
```
