...
Analyze codebase for real bugs, logic errors, and runtime issues. Categorizes findings by criticality with specific file locations and actionable fix suggestions. Use this to identify actual functional problems that will cause runtime failures, not theoretical concerns.

## Arguments

### Definitions

- **`[context]`** (optional): Specific areas or concerns to focus on, or directories/files to analyze. Defaults to analyzing the entire codebase.
