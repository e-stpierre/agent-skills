...
"""
Reinstall all plugins from the local marketplace.
...
Usage:
    ...
    uv run .claude/re-install-plugins.py interactive-sdlc
"""
...
def parse_args(all_plugins: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=f"""
Supported plugins: {", ".join(all_plugins)}

Examples:
  %(prog)s                    # Reinstall all plugins
  %(prog)s interactive-sdlc   # Reinstall only interactive-sdlc
""",
    )
    parser.add_argument(
        "plugins",
    ...
    if args.plugins:
        invalid = [p for p in args.plugins if p not in all_plugins]
        if invalid:
            parser.error(f"Invalid plugin(s): {', '.join(invalid)}\nSupported: {', '.join(all_plugins)}")
...
def main():
    ...
    requested_plugins = args.plugins if args.plugins else all_plugins
    ...
    with staged_marketplace(repo_root, marketplace_name):
        ...
        print_step(2, 2, "Reinstall Claude Code Plugins")
